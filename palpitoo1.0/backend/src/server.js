...
// ✅ CORREÇÃO: GET /carreira/:id agora retorna todos os dados que o perfilpublico.html precisa
// Antes retornava apenas { ligas: [] }, causando zeros em toda a aba Carreira Histórica
app.get('/carreira/:id', async (req, res) => {
  try {
    const { id } = req.params;

    // Totais de carreira calculados direto dos palpites
    const stats = await pool.query(`
      SELECT
        COALESCE(SUM(pontos_obtidos), 0)                        AS pontos_total,
        COUNT(*)                                                AS palpites_total,
        COUNT(*) FILTER (WHERE pontos_obtidos = 3)              AS cravadas_total,
        COUNT(*) FILTER (WHERE pontos_obtidos = 1)              AS acertos_total,
        COUNT(*) FILTER (WHERE pontos_obtidos = 0
                          AND pontos_obtidos IS NOT NULL)        AS erros_total
      FROM palpites WHERE usuario_id = $1
    `, [id]);

    // Ligas que o usuário participou (com dados extras para o histórico)
    const ligas = await pool.query(`
      SELECT l.id_liga, l.nome, l.codigo_convite, l.status,
             l.resumo_encerramento, ul.criado_em AS data_entrada
      FROM ligas l
      JOIN usuarios_ligas ul ON l.id_liga = ul.liga_id
      WHERE ul.usuario_id = $1
      ORDER BY ul.criado_em ASC
...
await pool.query(
  `UPDATE ligas SET status = 'aguardando', resumo_encerramento = $2 WHERE id_liga = $1`,
  [liga.id_liga, JSON.stringify(resumo)]
);
console.log(`⏸️ Liga "${liga.nome}" (ID ${liga.id_liga}) — temporada concluída, resumo salvo.`);
          }
        }
      }
    } catch (e) {
